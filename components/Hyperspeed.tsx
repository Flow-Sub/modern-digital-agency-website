...
class CarLights {
  ...
  // Fix: Changed from THREE.Color to number to align with common hex color usage in the component
  ...
  constructor(
    webgl: App,
    options: HyperspeedOptions,
    // Fix: Changed from THREE.Color to number
    colors: number[] | number,
    speed: [number, number],
    fade: THREE.Vector2
  ) {
  ...
  init() {
    ...
    if (Array.isArray(this.colors)) {
      ...
      // Fix: Removed invalid cast 'as number' since the property type is now 'number | number[]'
...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = (options.colors.sticks as any).map((c: any) => new THREE.Color(c));
    } else {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    container.appendChild(myApp.renderer.domElement);
